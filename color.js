function show(color){
    var blockWidth = $("body").width()  / 16;
    var blockHeight = blockWidth;

    var body = d3.select("body")
    var div = body.selectAll("div")

    for (var i = 0; i < color.member.length; i++) {
        var colorBlock = div.data( [ color.member[i] ], function(colorMember) { return colorMember;} )
                                            .enter()
                                            .append("div")
                                            .attr("class", "color")
                                            .style("width", blockWidth)
                                            .style("height", blockHeight)
                                            .style("background-color", function(c) { return c.rgbValue; });
        var colorBlockSvgContainer = colorBlock.append("svg");
        var colorBlockTexts = colorBlockSvgContainer.selectAll("text").data( [ color.member[i] ], function(c) { return c;} ).enter()

        colorBlockTexts.append("text")
                                    .attr("x", "10")
                                    .attr("y", "10")
                                    .text(function(c) { return c.rgbValue;} )
                                    .attr("font-size", "10px")
                                    .attr("fill", function(d) { return "#" + (Number("0x666666") + Number("0x" + d.rgbValue.substr(1))).toString(16).toUpperCase(); });
        colorBlockTexts.append("text")
                                    .attr("x", "10")
                                    .attr("y", "30")
                                    .text(function(c) { return c.description;} )
                                    .attr("font-size", "10px")
                                    .attr("fill", function(d) { return "#" + (Number("0x666666") + Number("0x" + d.rgbValue.substr(1))).toString(16).toUpperCase(); });
    };
}
